...
function calculateTaskProduction() {
    const taskDuration = parseInt(document.getElementById('task_duration').value, 10);
    const timeCells = document.querySelectorAll('#shift_table .time-left');
    let totalMinutes = 0;
    let memberMinutes = [];
    timeCells.forEach(cell => {
    ...
    const totalTasks = tasksPerMember.reduce((a, b) => a + b, 0);
    const averageTasksPerMember = memberMinutes.length > 0 ? (totalTasks / memberMinutes.length).toFixed(1) : 0;
    
    // Update results
    document.getElementById('total_minutes').textContent = Math.round(totalMinutes);
    document.getElementById('tasks_per_member').textContent = averageTasksPerMember;
    document.getElementById('total_tasks').textContent = totalTasks;
    
    // Enable export button if there are results
    document.getElementById('export_lead_time_btn').disabled = totalMinutes === 0;
    
    // Math breakdown
    let breakdown = `Total Available Minutes = ${Math.round(totalMinutes)} (sum of minutes left for each team member)`;
    breakdown += `<br>Task Duration = ${taskDuration} minutes`;
    breakdown += `<br>Tasks per Team Member = Available Minutes for each member / Task Duration`;
    breakdown += `<br>Total Tasks Team Can Produce = Sum of tasks per team member = ${totalTasks}`;
    breakdown += `<br>Average Tasks per Member = Total Tasks / Number of Members = ${averageTasksPerMember}`;
    document.getElementById('math_breakdown').innerHTML = breakdown;
}
