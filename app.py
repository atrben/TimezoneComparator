...
TZ_ABBREVIATIONS = {
    ...
    'America/Bogota': 'COT', 'Africa/Johannesburg': 'SAST', 'Pacific/Auckland': 'NZST/NZDT',
...
def get_full_timezone(tz_abbr):
    # Split the input if it contains a slash to get the base abbreviation
    tz_abbr_base = tz_abbr.split('/')[0].strip() if '/' in tz_abbr else tz_abbr
    mapping = {
        ...
        'NZDT': 'Pacific/Auckland', 'NZST': 'Pacific/Auckland', 'WAT': 'Africa/Lagos', 'CEST': 'Europe/Paris', 'EAT': 'Africa/Nairobi',
    ...
    tz_abbr_base = tz_abbr_base.upper()
    return mapping.get(tz_abbr_base, tz_abbr if tz_abbr in pytz.all_timezones else None)
...
def compare():
    try:
        base_tz_input = request.form.get('base_timezone', '').strip()
        base_tz = get_full_timezone(base_tz_input)
        ...
        for orig_tz in unique_original_tzs_ordered:
            ...
            if mapped_tz and mapped_tz in pytz.all_timezones and mapped_tz not in processed_tzs:
                ...
                skipped_timezones.append({
                    ...
                    'current_time': 'N/A',
                    'count': tz_counts.get(orig_tz, 1),
                    'error': 'Unrecognized or unmapped timezone'
                })
...
def calculate_shift():
    data = request.form
    base_tz_input = data.get('base_timezone', 'UTC')
    base_tz = get_full_timezone(base_tz_input)
    if not base_tz or base_tz not in pytz.all_timezones:
        return jsonify({'error': f'Invalid base timezone: {base_tz_input}'}), 400
    ...
    try:
        ...
        return jsonify({'error': f'Invalid base timezone: {base_tz}'}), 400
